...
st.info("Upload training spectra for multivariate model calibration. **Use at least 7 spectra for reasonable results**. "
        "Define stock solution concentrations (mg/mL) and mixing volumes (mL).")
...
num_train_mixtures = st.sidebar.number_input("Number of Training Mixtures", min_value=3, max_value=50, value=5)
...
if train_spectra:
    ...
    mixing_df = pd.DataFrame(0, index=[f"Sample {i+1}" for i in range(num_train_mixtures)], columns=columns)
...
if train_button:
    if not train_spectra:
        ...
        try:
            # Align all spectra to the same wavelength range
            wavelengths = sorted(list(set.intersection(*[set(df['Wavelength']) for df in train_spectra.values()])))
            wavelengths = np.array(wavelengths)

            X_train = np.array([df.set_index('Wavelength').reindex(wavelengths)['Absorbance'].values for df in train_spectra.values()])

            # Calculate real sample concentrations
            ...
            y_train = calculate_sample_concentrations(stock_conc_values, volumes_only, total_volumes[:, np.newaxis])

            # PLS Model
            pls = PLSRegression(n_components=min(5, X_train.shape[0]-1, X_train.shape[1]))
...
if analyze_button:
    if not (train_spectra and measurement_spectra):
        ...
        try:
            ...
            # Predict on measurement data (triplicates)
            X_measure = np.array([df.set_index('Wavelength').reindex(wavelengths)['Absorbance'].values for df in measurement_spectra.values()])
            y_pred = pls.predict(X_measure)

            # Calculate Mean and Std Dev for triplicates

...
def calculate_sample_concentrations(stock_concentrations, volumes, total_volume):
    concentrations = volumes.mul(stock_concentrations, axis=1).div(total_volume, axis=0)
...
if train_button:
    if not train_spectra:
        ...
        try:
            ...
            sample_names = list(train_spectra.keys())
            X_train = np.array([
                np.interp(wavelengths, train_spectra[name]['Wavelength'], train_spectra[name]['Absorbance'])
                for name in sample_names
            ])

            stock_conc_series = pd.Series(stock_concentrations_df["Stock Concentration (mg/mL)"].values, index=components)
            volumes_only = mixing_df.loc[sample_names, components]
            total_volumes = volumes_only.sum(axis=1) + mixing_df.loc[sample_names, "Water"]

            if not np.allclose(total_volumes.values, total_volumes.values[0]):
                st.error("All mixtures must have the same total volume. Please adjust the volumes in the table.")
            else:
